...
const EditDocumentPage: React.FC = () => {
  ...
  const [author, setAuthor] = useState('技术委员会');
  const [categoryId, setCategoryId] = useState('');
  const [teamId, setTeamId] = useState('');
  const [visibility, setVisibility] = useState('private');
  const [tags, setTags] = useState<string[]>([]);
  const [inputTag, setInputTag] = useState('');
  const [loading, setLoading] = useState(true); // 编辑页面初始为true，因为需要加载文档
  const [documentId, setDocumentId] = useState<string>(''); // 存储文档ID
  ...
  const {
    ...
    documentId: id, // 编辑已有文档，使用URL参数中的文档ID
    formData: autoSaveFormData,
  });

  // 恢复草稿：将 localStorage 中的草稿数据填充到表单
  useEffect(() => {
    // 仅在加载完远程文档后、且有可恢复的草稿时才处理
  ...
  const fetchDocument = async (docId: string) => {
    try {
      setLoading(true);
      console.log('🔄 开始加载文档，ID:', docId);
      console.log('🔗 API路径: `/document/documents/${docId}`');

      const data = await documentService.getDocument(docId);
      console.log('✅ 加载文档成功，返回数据:', data);
      console.log('📋 数据类型检查:', {
      ...
      console.log('🏷️ 处理后的tags:', processedTags);

      // 处理categoryId - 后端返回的是数字，需要转换为字符串以匹配select的value
      const processedCategoryId = data.categoryId ? String(data.categoryId) : '';
      console.log('📁 处理后的categoryId:', processedCategoryId, '类型:', typeof processedCategoryId);

      // 处理allowComment字段 - 后端返回的是数字(0或1)
      const processedAllowComments = data.allowComment !== undefined ? data.allowComment === 1 : true;
      console.log('💬 处理后的allowComments:', processedAllowComments);

      // 处理author信息 - 后端返回的是AuthorVO对象
      let processedAuthor = '技术委员会';
      if (data.author) {
      ...
      console.log('👤 处理后的author:', processedAuthor);
      ...
      setTitle(data.title || '');
      console.log('✅ Title设置完成:', data.title);

      setContent(data.content || '');
      console.log('✅ Content设置完成，长度:', data.content?.length || 0);

      setSummary(data.summary || '');
      console.log('✅ Summary设置完成:', data.summary);

      setAuthor(processedAuthor);
      setCategoryId(processedCategoryId);

      // 处理visibility - 映射到isPublic字段
      // is_public: 1=全员可见(public), 0=团队可见(team)
      const visibilityValue = data.isPublic !== undefined
        ? (Number(data.isPublic) === 1 ? 'public' : 'team')
        : (data.visibility === 1 ? 'public' : 'team');
      setVisibility(visibilityValue);

      // teamId字段在数据库中不存在，设为空
      setTeamId('');

      setTags(processedTags);
      console.log('✅ Tags设置完成:', processedTags);
      console.log('✅ Visibility设置完成:', visibilityValue, '(isPublic:', data.isPublic, ')');

      setAllowComments(processedAllowComments);
      setAllowEdit(false); // 后端没有这个字段，使用默认值
      setAiIndex(true); // 后端没有这个字段，使用默认值
      setSaveOption(data.status === 1 || data.status === 3 ? 'submit_review' : 'draft');
      console.log('✅ 其他字段设置完成');
      ...
      message.error('加载文档失败：' + (error as any)?.message || '未知错误');
      // 可以在这里添加导航回退逻辑
      // navigate('/documents');
    } finally {
      setLoading(false);
  ...
  const flattenCategories = (tree: any[]): any[] => {
    ...
    const traverse = (nodes: any[], level: number = 0) => {
      nodes.forEach((node, index) => {
        ...
        result.push({
          id: String(node.id), // 转换为字符串以匹配文档数据的categoryId
          name: node.name,
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      const documentData = {
        ...
        summary,
        categoryId: categoryId || undefined,
        tags: Array.isArray(tags) ? tags.join(',') : tags, // 转换为逗号分隔的字符串
        status: 0, // 0-草稿
        documentType: 1, // 1-文章
        allowComment: allowComments ? 1 : 0,
        source: 1, // 1-原创
        ...
        isRecommend: 0,
        sort: 0,
        isPublic: visibility === 'public' ? 1 : 0, // 映射visibility到isPublic
      };

      console.log('更新文档草稿，请求数据：', documentData);
      const result = await documentService.updateDocument(String(documentId), documentData);
      console.log('更新文档草稿成功，返回结果：', result);
      clearDraft();
      message.success('草稿保存成功！');
      window.open(`/documents/${documentId}`, '_blank');
    } catch (error) {
      console.error('更新文档草稿失败：', error);
  ...
  const handleSubmitForReview = async () => {
    ...
    try {
      ...
      const documentData = {
        ...
        categoryId: categoryId || undefined,
        tags: Array.isArray(tags) ? tags.join(',') : tags, // 转换为逗号分隔的字符串
        // 不设置 status，由后端 submitForReview 统一管理状态变更
        documentType: 1, // 1-文章
        allowComment: allowComments ? 1 : 0,
        source: 1, // 1-原创
        isTop: 0,
        ...
        <div style={{
          ...
          <div>
            <div style={{
              ...
              {from === 'drafts' ? '草稿箱 / 编辑文档' : '文档中心 / 编辑文档'}
            </div>
            <h1 style={{
              fontSize: '24px',
              fontWeight: '700',
