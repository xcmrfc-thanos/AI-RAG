...
-- 存储配置
(2000000000000000006, 's3.endpoints', 'http://localhost:8200', 'json', 'STORAGE', 'S3兼容对象存储端点', 1),
(2000000000000000007, 's3.bucket', 'knowledge-docs', 'string', 'STORAGE', 'S3存储桶', 1),
...
-- RAG 检索（Settings RAG；热读 rag.retrieval.*）
...
-- Agent（Settings AGENT；热读 agent.timeouts.*）
