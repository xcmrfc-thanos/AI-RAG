...
public class CommentServiceImpl extends ServiceImpl<CommentMapper, Comment> implements CommentService {
    ...
    public Long createComment(CommentCreateDTO dto) {
        ...
        if (userAvatar == null || userAvatar.isEmpty()) {
            userAvatar = documentUserClient.getUserAvatar(userId);
        }
        ...
        coreStatisticsProjectionPublisher.publishCommentUpsert(comment.getId(), userId, dto.getDocumentId());
    ...
    public Boolean deleteComment(Long commentId) {
        ...
        if (count > 0) {
            ...
            coreStatisticsProjectionPublisher.publishCommentDelete(commentId);
        }
    ...
    public Boolean likeComment(Long commentId) {
        ...
        Integer count = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM kb_like WHERE target_id = ? AND user_id = ? AND target_type = 2",
                Integer.class,
                commentId, userId
        );
        ...
        // 添加点赞记录
        jdbcTemplate.update(
                "INSERT INTO kb_like (id, target_id, user_id, target_type, created_at) VALUES (?, ?, ?, 2, NOW())",
                SnowflakeIdGenerator.getInstance().nextId(), commentId, userId
        );

        // 更新评论点赞数
        jdbcTemplate.update(
                "UPDATE kb_comment SET like_count = like_count + 1 WHERE id = ?",
                commentId
        );
    ...
    public Boolean unlikeComment(Long commentId) {
        ...
        int count = jdbcTemplate.update(
                "DELETE FROM kb_like WHERE target_id = ? AND user_id = ? AND target_type = 2",
                commentId, userId
        );
    ...
    private Set<Long> queryLikedCommentIds(Long userId, Set<Long> commentIds) {
        ...
        List<Long> likedIds = jdbcTemplate.queryForList(
                "SELECT target_id FROM kb_like WHERE user_id = ? AND target_type = 2 AND target_id IN (" + placeholders + ")",
                Long.class,
                params.toArray()
        );
