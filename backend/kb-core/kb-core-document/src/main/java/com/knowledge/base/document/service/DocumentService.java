...
public interface DocumentService extends IService<Document> {
     ...
     *
     * <p>适用于大文件分片/秒传后：前端已拿到 {@code FileMetadata.id}，
     * 服务端拉取字节走既有解析管线，不再重复上传。</p>
     *
     * @param fileId 文件管理元数据 ID（{@code FileMetadata.id}）
     * @return 与 {@link #uploadAndCreateDocument} 相同结构的结果 Map
     */
    Map<String, Object> createFromStoredFile(Long fileId);
