...
public class FileManagementController {
    ...
    /**
     * 按内容哈希登记文件管理元数据（秒传/分片 merge 后调用，不重复上传）。
     *
     * <p>服务端通过 kb-file check-hash 解析 fileUrl；客户端传入的 fileUrl 忽略。</p>
     *
     * @param dto 登记参数
     * @return 文件元数据
     */
    ...
    @PreAuthorize("hasAnyAuthority(T(com.knowledge.base.document.constants.DocumentPermissionConstants).DOCUMENT_CREATE, T(com.knowledge.base.document.constants.DocumentPermissionConstants).DOCUMENT_EDIT)")
    public Result<FileMetadataVO> registerStored(
            @Parameter(description = "登记参数", required = true)
            @RequestBody RegisterStoredDTO dto) {

        Long userId = UserContext.getCurrentUserId();
        log.info("登记已存文件请求：userId={}, fileName={}, sha256={}",
                userId, dto != null ? dto.getOriginalFileName() : null,
                dto != null ? dto.getFileSha256() : null);

        FileMetadata metadata = fileManagementService.registerStored(dto, userId);
        return Result.success("登记成功", convertToVO(metadata));
    }
    ...
    public void streamFile(
        ...
        log.info("流式播放文件请求：fileId={}, download={}", fileId, download);
