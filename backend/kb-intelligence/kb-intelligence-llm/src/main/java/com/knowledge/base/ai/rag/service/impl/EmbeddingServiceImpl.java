...
public class EmbeddingServiceImpl implements EmbeddingService {
    ...
    public float[] embed(String text) {
        ...
        if (embeddingModel == null) {
            if (devStubEnabled) {
                return buildDeterministicVector(text);
            }
        ...
        if (embeddingModel == null) {
            if (devStubEnabled) {
                ...
                for (String text : texts) {
                    vectors.add(buildDeterministicVector(text));
                }
                return vectors;
            }
    ...
    /**
     * 本地开发 Stub：基于文本哈希生成归一化非零向量，满足 ES cosine 索引要求。
     */
    private float[] buildDeterministicVector(String text) {
        int dim = ragProperties.getEmbedding().getDimension();
        float[] vector = new float[dim];
        byte[] hash = DigestUtils.sha256(text == null ? "" : text);
        for (int i = 0; i < dim; i++) {
            vector[i] = ((hash[i % hash.length] & 0xFF) / 128.0f) - 1.0f;
        }
