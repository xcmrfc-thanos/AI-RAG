...
**进展（2026-07-31）：** Task 0～4 完成（`kb-mcp` + Gateway `/api/mcp` + `verify-mcp-readonly.ps1`）。启用：`mcp.server.enabled=true` 并导入网关路由。
...
| **M2** | 完成 M1 未选项，或在有宿主方时做 P3 Embed | P3 自嵌已交付；**P1 首期只读 MCP 已交付**（默认关） |
...
4. **可选运维：** P1 Live 联调（启 `kb-mcp` + 导入路由 + `-Live -EnableProbe`）。~~P1.1 `check_sensitive`~~ **已取消**。
