...
| 里程碑 | 内容 | 退出条件 |
|--------|------|----------|
| **M0** | P0 需求与边界收敛 | 决定 P1/P2/P3 哪些有真实使用方 |
| **M1** | P1 MCP Server 或 P2 `@workflow` 中价值更明确者 | 独立计划完成统一 Gate |
| **M2** | 完成 M1 未选项，或在有宿主方时做 P3 Embed | 前一阶段稳定且无安全遗留 |
...
2. **M1 当前项：** 为 **P2 结构化 `@workflow`** 写独立 design + implementation plan 并实施（内部产品获准）。
3. P1 只读 MCP Server：待出现外部消费方与身份约定后再开 design；当前不因路线图自动开工。
4. Embed、MCP Client、Cron 均按各自 Gate 决定是否继续，不因已列入路线图而自动开工。
