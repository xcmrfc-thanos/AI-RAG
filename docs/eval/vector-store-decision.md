...
| 项 | 结论 |
|----|------|
| **唯一决策** | **继续 Elasticsearch** 作为生产向量与混合检索默认后端 |
...
| Qdrant（2026-07-18） | **可选旁路**：`rag.qdrant.enabled=false` 默认关；开启后 ES 双写 + hybrid=ES BM25 + Qdrant dense（融合默认 RRF，可选 weighted）。**不是**替换 ES 的唯一生产后端 |
| 任务 74 | 以「无新增默认向量后端」结项；Qdrant 为开关增强，不改变默认路径 |
